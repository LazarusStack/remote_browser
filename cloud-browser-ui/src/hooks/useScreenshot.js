...
export function useScreenshot(socketRef, activeTab) {
  ...
  useEffect(() => {
    ...
    const processFrameQueue = async () => {
      ...
      // First, sort by frameId ascending
      frameQueueRef.current.sort((a, b) => a.frameId - b.frameId);
      ...
      // Find the latest frame that is newer than what we currently have
      // We want to skip any frames that arrived late (older than current)
      let latestFrameIndex = -1;
      const currentFrameId = latestScreenshotRef.current?.frameId || 0;

      for (let i = frames.length - 1; i >= 0; i--) {
        if (frames[i].frameId > currentFrameId) {
          latestFrameIndex = i;
          break;
        }
      }

      if (latestFrameIndex === -1) {
        // All frames in queue are older than what we have shown. Drop them.
        droppedFrames += frames.length;
        isProcessingRef.current = false;
        return;
      }

      // We found the best new frame. 
      // All frames before it are outdated (skipped).
      // All frames after it (in the original sorted list, which we traversed backwards) were... wait.
      // If we traverse backwards, the first one > current is the absolute latest available.

      droppedFrames += (frames.length - 1); // We are dropping everything except one
      const latestFrame = frames[latestFrameIndex];
